 ...
 *
 * - Bots: preserve snapshots and stable first-seen order. Connection status is unknown
 *   here: hull presence is not transport evidence. The admin roster supplies it.
...
export function reconcile(
  ...
  // disappear without guessing their network state.
  for (const [id, card] of prev) {
    next.set(id, { ...card, connected: null });
  }
