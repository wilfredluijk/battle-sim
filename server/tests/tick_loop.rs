...
use naval_server::room::{run_room, Room};

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn run_room_ticks_at_configured_rate() {
    let (tx, rx) = broadcast::channel::<()>(2);
    let room = Room::new("test".into(), 1000.0, 1000.0, 42);
    let handle = tokio::spawn(run_room(room, 10, rx));
...
async fn run_room_shuts_down_promptly() {
    let (tx, rx) = broadcast::channel::<()>(2);
    let room = Room::new("test".into(), 1000.0, 1000.0, 42);
    let handle = tokio::spawn(run_room(room, 100, rx));
